...
function newDialogShow(url,tablename) {
    ...
    $.post(
        ...
        function(data) {
            $('#newResult').html(data);
            $("#EnfermedadesPino_tipo_enfermedad").prop("disabled", true);
            $("#EnfermedadesPino_opcion_id").prop("disabled", true);
            $.post(
                '/lifn/default/getavailabletreesp',
                {muestreo:muestreo},
                function(info) {
                    $('#newResult').append(info);
                }
            );
            $("#EnfermedadesPino_numArbol").focusout(function()
            {
                var numArbol = $("#EnfermedadesPino_numArbol").val();
                if(numArbol != "")
                {
                    $("#EnfermedadesPino_tipo_enfermedad").prop("disabled", false);
                    var request = $.getJSON("/lifn/default/getRemainingDiseasesEucalyptus?numArbol="+numArbol, function(data){
                        $("#EnfermedadesPino_tipo_enfermedad").html("");
                        var options = {};
                        $.each(data, function(key, val) {
                            if(key == "error")
                            {
                                ...
                                $("#EnfermedadesPino_tipo_enfermedad").prop("disabled", true);
                                $("#EnfermedadesPino_tipo_enfermedad").html("");
                                $("#EnfermedadesPino_opcion_id").html("");
                                $("#EnfermedadesPino_numArbol").val("");
                            }
                            else
                            {
                            $("#EnfermedadesPino_tipo_enfermedad").append("<option value=" + key + ">" + val + "</option>");
                            }
                        });      
                    });
                }
            ...
            $("#EnfermedadesPino_tipo_enfermedad").change(function() {
                var idPlant = $("#EnfermedadesPino_tipo_enfermedad").val();
                if (idPlant == ""){
                    $("#EnfermedadesPino_opcion_id").prop("disabled", true);
                    $("#EnfermedadesPino_opcion_id").html("");
                } else {
                    $("#EnfermedadesPino_opcion_id").prop("disabled", false);
                    var request = $.getJSON("/lifn/default/subSelectDataSanidadP?id="+idPlant, function(data){
                        $("#EnfermedadesPino_opcion_id").html("");
                        var options = {};
                        $.each(data, function(key, val) {
                            $("#EnfermedadesPino_opcion_id").append("<option value=" + key + ">" + val + "</option>");
                        });      
                    });
                }
            });
